...
import { Badge, Button, Card, ErrorText, Input, Shell } from "@/components/Shell";
...
const emptyForm: Form = { name: "", points: "5", is_rental: false, rental_minutes: "5", max_per_group: "", global_inventory: "", image_url: null };
...
const toForm = (p: Product): Form => ({
  ...
  global_inventory: p.global_inventory === null ? "" : String(p.global_inventory),
...
const fromForm = (f: Form) => ({
  ...
  global_inventory: f.global_inventory === "" ? null : Number(f.global_inventory),
...
export default function AdminProductsPage() {
  ...
  const save = async () => {
    ...
    if (!Number.isInteger(values.points) || values.points < 0) return setError("Points must be a whole number ≥ 0");
    if (values.is_rental && (!Number.isInteger(values.rental_minutes) || values.rental_minutes < 1))
      return setError("Rental minutes must be a whole number ≥ 1");
    if (values.max_per_group !== null && (!Number.isInteger(values.max_per_group) || values.max_per_group < 1))
      return setError("Max per group must be a whole number ≥ 1 (or blank)");
    if (values.global_inventory !== null && (!Number.isInteger(values.global_inventory) || values.global_inventory < 0))
    ...
    if (file) {
      try {
        ...
        setBusy(false);
        return setError(`Could not read image: ${e instanceof Error ? e.message : String(e)}`);
      }
    }
    const { error } =
      editing === "new"
        ? await supabase.from("order_products").insert(values)
        : await supabase.from("order_products").update(values).eq("id", editing!);
    setBusy(false);
  ...
  const remove = async (p: Product) => {
    ...
    const { error } = await createClient().from("order_products").delete().eq("id", p.id);
    if (error)
      setError(
        error.code === "23503"
    ...
    <Shell title="Admin · Products" right={<AdminNav />}>
      ...
      <div className="grid gap-6 md:grid-cols-[320px_1fr]">
        <Card className="h-fit space-y-3">
          <h2 className="font-semibold">{editing === "new" ? "New product" : editing ? "Edit product" : "Product"}</h2>
          {editing === null ? (
            ...
            <>
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                <span>Rental item (stock returns when the order is completed)</span>
              </label>
              {form.is_rental && (
                <label className="block space-y-1 text-sm">
              ...
              <label className="block space-y-1 text-sm">
                ...
                <Input type="number" min={1} placeholder="No limit" {...field("max_per_group")} />
              </label>
              <label className="block space-y-1 text-sm">
                <span>Global inventory (blank = unlimited)</span>
                <Input type="number" min={0} placeholder="Unlimited" {...field("global_inventory")} />
              </label>
                ...
                <tr key={p.id} className="border-t border-zinc-100 dark:border-zinc-800">
                  <td className="py-2 font-medium">
                    <div className="flex items-center gap-2">
                      <ProductImage src={p.image_url} alt="" className="h-10 w-10 shrink-0 rounded" />
                      {p.name}
                    </div>
                  </td>
                  ...
                  <td className="py-2">
                    <div className="flex flex-wrap gap-1">
                      {p.is_rental && <Badge tone="warn">Rent {p.rental_minutes} min</Badge>}
                      {p.max_per_group !== null && <Badge>Max {p.max_per_group}/group</Badge>}
                      {p.global_inventory !== null && <Badge>Stock {p.global_inventory}</Badge>}
                      {!p.is_rental && p.max_per_group === null && p.global_inventory === null && (
                        <span className="text-xs text-zinc-400">No limit</span>
                      )}
                    </div>
                  </td>
