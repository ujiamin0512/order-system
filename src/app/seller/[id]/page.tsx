...
type ItemRow = OrderItem & { order_products: Pick<Product, "name" | "is_rental" | "rental_minutes" | "image_url"> | null };

export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [order, setOrder] = useState<(Order & { order_groups: { name: string } | null }) | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const [o, i] = await Promise.all([
      supabase.from("order_orders").select("*, order_groups(name)").eq("id", id).single(),
      supabase.from("order_order_items").select("*, order_products(name, is_rental, rental_minutes, image_url)").eq("order_id", id),
    ]);
      ...
      {!order ? (
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_280px]">
                  ...
                  <tr key={it.id} className="border-t border-zinc-100 dark:border-zinc-800">
                    <td className="py-2">
                      <div className="flex items-center gap-2">
                        <ProductImage src={it.order_products?.image_url} alt="" className="h-10 w-10 shrink-0 rounded" />
                        <span>
                          {it.order_products?.name ?? "Unknown"}{" "}
                          {it.order_products?.is_rental && (
                            <Badge tone="warn">Rent {it.order_products.rental_minutes} min</Badge>
                          )}
                        </span>
                      </div>
                    </td>
                    <td className="py-2 text-right">{it.quantity}</td>
                    <td className="py-2 text-right">{it.quantity * it.point_price}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-zinc-200 font-semibold dark:border-zinc-700">
                  ...
                  <td />
                  <td className="pt-2 text-right">{order.total_points} pts</td>
                </tr>
              </tfoot>
            </table>
          </Card>
          ...
          <Card className="h-fit space-y-3">
            <p className="text-lg font-semibold">{order.order_groups?.name ?? "Unknown group"}</p>
